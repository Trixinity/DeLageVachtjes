...
function getCommandDifferences(existingCommand, apiData, guildCommand) {
  ...
  if (existingCommand.type !== v10.ApplicationCommandType.ChatInput && existingCommand.type) {
    if (contextMenuTypes.includes(existingCommand.type ?? v10.ApplicationCommandType.ChatInput)) {
      ...
      if ((existingCommand.default_permission ?? true) !== (casted2.default_permission ?? true)) {
        differences.push({
          key: "defaultPermission",
          original: String(existingCommand.default_permission ?? true),
          expected: String(casted2.default_permission ?? true)
        });
      }
      ...
      if (!originalLocalizedNames2 && expectedLocalizedNames2) {
        ...
        differences.push(...reportLocalizationMapDifferences(originalLocalizedNames2, expectedLocalizedNames2, "nameLocalizations"));
      }
    }
    return differences;
  }

...
function isAnyInteraction(messageOrInteraction) {
  return !(messageOrInteraction instanceof discord_js.Message);
}
...
function actionIsButtonOrMenu(action) {
  return action.type === discord_js.Constants.MessageComponentTypes.SELECT_MENU || action.type === "SELECT_MENU" || action.style !== "LINK" && action.style !== discord_js.Constants.MessageButtonStyles.LINK;
}
__name(actionIsButtonOrMenu, "actionIsButtonOrMenu");
function isMessageButtonInteraction(interaction) {
  return interaction.type === discord_js.Constants.MessageComponentTypes.BUTTON || interaction.type === "BUTTON";
}
...
function createPartitionedMessageRow(components) {
  const [messageButtons, selectMenus] = utilities.partition(components, isMessageButtonComponent);
  const chunkedButtonComponents = utilities.chunk(messageButtons, 5);
  const messageButtonActionRows = chunkedButtonComponents.map(
    (componentsChunk) => new discord_js.MessageActionRow().setComponents(componentsChunk)
  );
  const selectMenuActionRows = selectMenus.map(
    (component) => new discord_js.MessageActionRow().setComponents(component)
  );
...
var _PaginatedMessage = class {
  ...
  async run(messageOrInteraction, target) {
    if (!messageOrInteraction.channel) {
      ...
      if (shouldEmitWarning && !isInteraction && messageOrInteraction.client.options.partials?.includes("CHANNEL") && new discord_js.Intents(messageOrInteraction.client.options.intents).has(discord_js.Intents.FLAGS.DIRECT_MESSAGES)) {
        shouldEmitWarning = false;
      }
    ...
    if (this.pages.length > 1) {
      const messageComponents = await Promise.all(
        [...this.actions.values()].map(async (interaction) => {
          return isMessageButtonInteraction(interaction) ? new discord_js.MessageButton(interaction) : new discord_js.MessageSelectMenu({
  ...
  setUpCollector(channel, targetUser) {
    if (this.pages.length > 1) {
      this.collector = new discord_js.InteractionCollector(targetUser.client, {
        ...
        channel,
        interactionType: discord_js.Constants.InteractionTypes.MESSAGE_COMPONENT,
...
__publicField(PaginatedMessage, "defaultActions", [
  {
    ...
    type: discord_js.Constants.MessageComponentTypes.SELECT_MENU,
  ...
  },
  {
    ...
    style: "PRIMARY",
    emoji: "\u23EA",
    type: discord_js.Constants.MessageComponentTypes.BUTTON,
...
__publicField(PaginatedMessage, "wrongUserInteractionReply", /* @__PURE__ */ __name((targetUser) => ({
  content: `Please stop interacting with the components on this message. They are only for ${discord_js.Formatters.userMention(targetUser.id)}.`,
  ephemeral: true,
...
var PaginatedFieldMessageEmbed = class extends PaginatedMessage {
  constructor() {
    super(...arguments);
    __publicField(this, "embedTemplate", new discord_js.MessageEmbed());
  ...
  resolveTemplate(template) {
    ...
    if (utilities.isFunction(template)) {
      return template(new discord_js.MessageEmbed());
    }
    return new discord_js.MessageEmbed(template);
  }
};
...
var canReadMessagesPermissions = new discord_js.Permissions(["VIEW_CHANNEL"]);
...
var canSendMessagesPermissions = new discord_js.Permissions([canReadMessagesPermissions, "SEND_MESSAGES"]);
...
var canSendEmbedsPermissions = new discord_js.Permissions([canSendMessagesPermissions, "EMBED_LINKS"]);
...
var canSendAttachmentsPermissions = new discord_js.Permissions([canSendMessagesPermissions, "ATTACH_FILES"]);
...
var canReactPermissions = new discord_js.Permissions([canSendMessagesPermissions, "READ_MESSAGE_HISTORY", "ADD_REACTIONS"]);
...
var canRemoveAllReactionsPermissions = new discord_js.Permissions([canReadMessagesPermissions, "READ_MESSAGE_HISTORY", "MANAGE_MESSAGES"]);
...
var canJoinVoiceChannelPermissions = new discord_js.Permissions(["CONNECT"]);
